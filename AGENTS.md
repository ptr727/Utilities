...
- **`dotnet/nbgv` is consumed via `@master`, never SHA-pinned.** Its upstream tag stream lags `master` substantially, so a SHA pin would only draw spurious Dependabot downgrade PRs; the rationale is documented inline in [`build-release-task.yml`](./.github/workflows/build-release-task.yml). Do not convert it to a SHA pin.
