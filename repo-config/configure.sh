...
# Secrets by store (names only; values are never readable via the API). The App credentials must be set
# in BOTH stores: the merge-bot reads them from Actions on a normal PR, but from the
# Dependabot store when it acts on a Dependabot PR, because Dependabot-triggered runs are given the
# Dependabot secret store, not Actions secrets. NUGET_USERNAME is Actions-only (publishing is not a Dependabot
# run); publishing is keyless via OIDC trusted publishing, so there is no NUGET_API_KEY.
REQUIRED_ACTIONS_SECRETS=(NUGET_USERNAME CODEGEN_APP_CLIENT_ID CODEGEN_APP_PRIVATE_KEY)
...
cmd_check() {
  ...
  note "verify manually: NuGet.org trusted-publishing policy (owner ptr727, repo Utilities, workflow build-release-task.yml)"
